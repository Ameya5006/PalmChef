...
import { extractIngredients, mapFirstIngredientUsage } from "@/utils/ingredients";
...
const AddRecipeForm: React.FC = () => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const parsedIngredients = extractIngredients(rawText);
      const ingredients: Ingredient[] = mapFirstIngredientUsage(parsedIngredients, steps);
      addRecipe({
        id: crypto.randomUUID(),
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
      ...
      {loading && <p className="mt-2 text-sm text-sky-500">Processing…</p>}
